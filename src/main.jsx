...
import {
  ...
  Trash2, Link2, Upload, Menu, SlidersHorizontal, RefreshCw, Info
} from "lucide-react";
import "./styles.css";
...
function App(){
  ...
  useEffect(()=>{ if(active && store.active!==active.id){const n={...store,active:active.id};setStore(n);saveStore(n)} },[active]);
  useEffect(()=>{
...
function Welcome({onAdd}){
 return <section className="welcome">
   ...
   <div className="welcome-content">
    ...
    <p>Adicione uma playlist M3U/M3U8 e organize seus conteúdos em uma interface moderna.</p>
...
function AddPlaylist({onClose,onAdd}){
 const [tab,setTab]=useState("url"), [name,setName]=useState("Minha Playlist"), [url,setUrl]=useState(""), [loading,setLoading]=useState(false), [error,setError]=useState("");
 async function submit(){
   setError(""); if(!url.trim()){setError("Cole uma URL M3U/M3U8.");return}
   setLoading(true);
   try{
     // A playlist é buscada pelo endpoint serverless para evitar o bloqueio
     // de CORS que acontece quando o navegador acessa diretamente o provedor.
     ...
     const text=data.content || ""; const items=parseM3U(text,url.trim());
     if(!items.length) throw new Error("A playlist foi acessada, mas nenhum conteúdo foi encontrado.");
     onAdd({id:crypto.randomUUID(),name:name.trim()||"Minha Playlist",url:url.trim(),items,createdAt:Date.now()});
 ...
 async function file(e){
   ...
   try{const text=await f.text();const items=parseM3U(text,f.name);if(!items.length)throw new Error("Nenhum conteúdo encontrado.");onAdd({id:crypto.randomUUID(),name:name.trim()||f.name,url:"arquivo local",items,createdAt:Date.now()})}
   catch(e){setError(e.message)}finally{setLoading(false)}
 }
 return <div className="modal-backdrop"><div className="modal">
   <button className="modal-close" onClick={onClose}><X/></button><span className="eyebrow">NOVA PLAYLIST</span><h2>Adicionar conteúdo</h2><p className="modal-sub">Use uma URL M3U/M3U8 ou carregue um arquivo.</p>
   <label>Nome<input value={name} onChange={e=>setName(e.target.value)} placeholder="Minha Playlist"/></label>
   <div className="tabs"><button className={tab==="url"?"active":""} onClick={()=>setTab("url")}><Link2/> URL</button><button className={tab==="file"?"active":""} onClick={()=>setTab("file")}><Upload/> Arquivo</button></div>
   {tab==="url"?<label>URL M3U/M3U8<input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://exemplo.com/playlist.m3u"/></label>:<label className="filebox"><Upload size={22}/><span>Selecionar arquivo M3U</span><input type="file" accept=".m3u,.m3u8,.txt" onChange={file}/></label>}
   {error&&<div className="error">{error}</div>}
   {tab==="url"&&<button className="primary-btn full" disabled={loading} onClick={submit}>{loading?<><RefreshCw className="spin"/> Lendo playlist...</>:<><Plus/> Adicionar playlist</>}</button>}
 </div></div>
}
